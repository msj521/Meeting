...
//					console.log(Base64.decode(play_url.data[1]))
//					console.log(Base64.decode(play_url.data[2]))
//					console.log(Base64.decode(play_url.data[3]))
		var options = {
...
 
		...
		for(var i = 0; i < comment_list.length; i++) {
			if(comment_list[i].is_praise == 0) {
				$('<div class="review_ct">' +
					...
					'<div class="zan1 zanlist" id="' + comment_list[i].fid + '">' + comment_list[i].praise_num + '</div>' +
					'</div>' +
					'</div>').appendTo(".review_list");
			} else if(comment_list[i].is_praise == 1) {
				$('<div class="review_ct">' +
					'<div class="review_ct_img">' + '<img src="' + comment_list[i].web_image_url + '" />' + '</div>' +
					'<div class="review_ct_text">' +
					'<div class="review_ct_user">' + '<span class="review_ct_name">' + comment_list[i].nick_name + '</span>' + '<span class="review_ct_time">' + comment_list[i].create_time + '</span>' + '</div>' +
					'<div class="review_content">' + comment_list[i].content + '</div>' +
					'<div class="zan2 zanlist" id="' + comment_list[i].fid + '">' + comment_list[i].praise_num + '</div>' +
					'</div>' +
					'</div>').appendTo(".review_list");
			}
	...
	ajax_all_Filed("true", "true", "POST", trainingurl, "json", data, training); //调用函数
	...
	$(".fab_btn").click(function() {
		...
		function publish(data) {
			...
			//parent.location.reload(".review_list");
		}
		var data = {
			'training_id': training_id,
			'video_id': video_id,
	...
	$(".review_list").on('click', '.zanlist', function() {
		...
		function parise(data) {
			if(data.code == 200) {
				showLaert(data.msg)
				$(this).removeClass("zan1").addClass("zan2");
				$("body").on('click', '#alert_btn', function() {
					window.location.reload()
				})
			}
	...
	//$('.pulic_recor').css('height', $(window).height() - 110);

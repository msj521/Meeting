...
<div class="x-nav">
            <span class="layui-breadcrumb">
              <a><cite>考试管理</cite></a>
              <a><cite><?php echo (isset($title) && ($title !== '')?$title:'考试题目'); ?></cite></a>
            </span>
...
<div class="x-body">
    ...
    <div class="layui-form-pane" style="text-align: center;">
      <div class="layui-form-item" style="display: inline-block;">
        <div class="layui-input-inline">
          <input type="text" name="string" placeholder="产品名称/题目名称/创建者" autocomplete="off" class="layui-input" value="<?php echo $string; ?>">
        </div>
  ...
  <xblock>
    <button class="layui-btn" onclick="section_edit('添加','<?php echo (isset($edit) && ($edit !== '')?$edit:'/'); ?>','<?php echo (isset($index) && ($index !== '')?$index:'/'); ?>',0,'<?php echo (isset($fid) && ($fid !== '')?$fid:0); ?>')"><i class="layui-icon">&#xe608;</i>添加</button>
...
<script>
  ...
  for (var i = 0; i < dataList.data.length; i++) {
    ...
    if(dataList.data[i].subject_type==1){
        dataList.data[i].subject_type='单选题';
    }else if(dataList.data[i].subject_type==2){
        dataList.data[i].subject_type='多选题';
    }else if(dataList.data[i].subject_type==3){
        dataList.data[i].subject_type='判断题';
    }
  ...
  layui.use(['table','layer'],function(){
    ...
    table.render({
      ...
      ,cols: [[
        ...
        ,{field:'subject_name',title: '题目名称'}
        ,{field:'subject_type',title: '题目类型'}
        ,{field:'score',title: '题目分值'}
    ...
    table.on('tool(tableTool)', function(obj){
      ...
      if(obj.event === 'del'){
        ...
        section_edit('编辑','<?php echo (isset($edit) && ($edit !== '')?$edit:"/"); ?>','<?php echo (isset($index) && ($index !== '')?$index:"/"); ?>',data.fid,'<?php echo (isset($fid) && ($fid !== '')?$fid:0); ?>',0,0)
      }
    });
  })
  ...
  function x_section_consult(title,url,urls,id,pid,w,h) {
    ...
    if (pid == '' || pid == null || pid < 0) {
      ...
      layer.open({
        ...
        shade: 0.4,
        title: title,
        content: url + "?fid=" + id +"&exam_id=" + pid,
        cancel: function () {
          window.location.href = urls + "?fid=" + pid
        }
      });

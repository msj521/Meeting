...
<script>
		...
		function mtdownload(data){
			if(data.data.length){
				pdfurls = data.data[0].file_path;
				var download_title = data.data[0].download_title;
				if(pdfurls.substr(-3,3)=="pdf"){
					$('<a href="'+pdfurls+'" target="view_window" >'+download_title+'</a>').appendTo("#pdfname");
					createPdf(pdfurls)
				}else{
					$('<a href="https://view.officeapps.live.com/op/view.aspx?src='+pdfurls+'" target="view_window" >'+download_title+'</a>').appendTo("#pdfname");
					$('<iframe src="https://view.officeapps.live.com/op/view.aspx?src='+pdfurls+'" width="100%" height="750" frameborder="1"></iframe>').appendTo("#pdf2");
				}
			}
		}
		var data = {"convention_id":convention_id,"module":"B4"};

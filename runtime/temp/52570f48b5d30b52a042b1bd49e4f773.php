...
<body>
  ...
  <div class="x-body">
    <form class="layui-form x-center" action="<?php echo (isset($index) && ($index !== '')?$index:'/'); ?>" method="get">
      <input type="hidden" name="convention_id" value="<?php echo (isset($fids) && ($fids !== '')?$fids:0); ?>">
      <input type="hidden" name="download_type" value="<?php echo (isset($download_type) && ($download_type !== '')?$download_type:0); ?>">
  ...
  <xblock>
    <button class="layui-btn" onclick="member_edits('添加','<?php echo (isset($edit) && ($edit !== '')?$edit:'/'); ?>','<?php echo (isset($index) && ($index !== '')?$index:'/'); ?>','','<?php echo (isset($fids) && ($fids !== '')?$fids:0); ?>','<?php echo (isset($download_type) && ($download_type !== '')?$download_type:0); ?>')"><i
        class="layui-icon">&#xe608;</i>添加</button>
    <button class="layui-btn layui-btn-danger" onclick="member_del(-1,'<?php echo (isset($del) && ($del !== '')?$del:'/'); ?>','<?php echo (isset($index) && ($index !== '')?$index:'/'); ?>')"><i
...
<script>
  ...
  <script>
    ...
    for (var i = 0; i < dataList.data.length; i++) {
      ...
      dataList.data[i].file_size = dataList.data[i].file_size + 'kb';
      if (dataList.data[i].download_type == 7) {
        dataList.data[i].creator_id = '(管理员)' + dataList.data[i].user_name;
      } else {
        dataList.data[i].creator_id = '(普通用户)' + dataList.data[i].enduser_name;
      }
    }
    ...
    layui.use(['table', 'layer'], function () {
      ...
      table.render({
        ...
        , cols: [[
          ...
          , { field: 'convention_name', width: 160, title: '会议名称' }
          , { field: 'download_title', width: 160, title: '文件标题' }
          , { field: 'file_name', width: 160, title: '文件名称' }
          , { field: 'file_path', width: 160, title: '文件地址' }
          , { field: 'file_size', width: 100, title: '文件大小' }
          , { title: '预览', templet: '<div><a href="{{d.file_path}}" style="color:red" target="_blank">预览</a></div>' }
          , { field: 'record_status', width: 100, title: '状态' }
          , { field: 'create_time', width: 160, title: '创建时间' }
          , { field: 'creator_id', width: 160, title: '上传者' }
      ...
      table.on('tool(tableTool)', function (obj) {
        var data = obj.data;
        if (obj.event === 'del') {
          member_del(data.fid, '<?php echo (isset($del) && ($del !== '')?$del:' / '); ?>', '<?php echo (isset($index) && ($index !== '')?$index:' / '); ?>?fid=' + data.convention_id + '&B<?php echo (isset($download_type) && ($download_type !== '')?$download_type:0); ?>')
        } else if (obj.event === 'edit') {
          member_edits('编辑', '<?php echo (isset($edit) && ($edit !== '')?$edit:' / '); ?>', '<?php echo (isset($index) && ($index !== '')?$index:' / '); ?>', data.fid, data.convention_id, '<?php echo (isset($download_type) && ($download_type !== '')?$download_type:0); ?>')
        }
      });
    })
  </script>

$(function() {
    ...
    var exam_id = 0;
    var jump_url='<a href="' + pu_url + '">前往登录</a>';
    ...
    function training(data) {
        ...
        for (var i = 0; i < comment_list.length; i++) {
            ...
            $('<div class="review_ct">' +
                ...
                '<div class="review_content">' + comment_list[i].content + 
				'<span style="float:right;margin-top:5px;" class="' + zan + ' zanlist" id="' + comment_list[i].fid + '">' + comment_list[i].praise_num +'</span>'+
                '</div>' +
        ...
        function publish(data) {
            if(data.code == 200){
				...
				$('<div class="review_ct">' +
					...
					'<div class="review_content">' + data.data.content +
					'<span style="float:right;margin-top:5px;" class="zan1 zanlist" id="' + data.data.fid + '">' + data.data.praise_num +'</span>'+
					'</div>' +
					'</div>').prependTo(".review_list");
            }else if(cook_uid=="" || cook_uid==null){
				showLaert(data.msg + jump_url);
			}else{
				showLaert(data.msg);
			}
			$("#content").val("");

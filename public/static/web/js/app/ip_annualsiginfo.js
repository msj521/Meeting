	...
	function userinfo(data) {
			...
			if(data.data!=undefined && data.data.pay_state == "报名成功"){
				...
				//医院id
					...
					//支付
					if(data.data.pay_type ==5){
						$("#xians").addClass("active");
						$("#xians img").attr('src','/static/web/images/icon_onlineselected.png')
					}
					//调用公用加载职称
					credit_title();
...
//		}
		function userinfo(data) {
			if(data.code==414){
				...
				$("#gender").val(data.data.gender)
				$("#job_id").val(data.data.job_id);//职称id
				deparitmentss=data.data.deparitment_id;//
				credit_typess=data.data.credit_type;//职称1id
				credit_titless=data.data.credit_title;//职称2id
				$("#basic_level").val(data.data.basic_level);
				education_idss=data.data.education_id;//学历
		...
		function dingdan(data) {
			if(data.code == 200) {
				...
				$("#alert_btn").click(function() {
					if(invoice_status==1&&tax_num==""){
						window.location.href = "/invoices?convention_id=" + convention_id;
					}else{
...
//			}
	...
	//存入本地
	var baseList = new Object();
	$("#next_btn").click(function(){
		if($("#email").val()== ""||$("#email").val().search(/^[\w-]+(\.[\w-]+)*@[\w-]+(\.[\w-]+)+$/) == -1) {
			alert("邮箱不能为空或填写错误！");
			return false;
		}
		...
		if($("#card").val() == ""||$("#card").val().search(/^(\d{15}$|^\d{18}$|^\d{17}(\d|X|x))$/) == -1) {
			alert("身份证不能为空或填写错误！");
			return false;
		}
		if($("#unit_tel_guo").val() == "") {
		...
		if($("#tel").val() == ""|| $("#tel").val().search(/^(13|15|17|18|19)\d{9}$/) == -1) {
			alert("手机号码不能为空或填写错误！");
			return false;
		}
		var unit_tel_guo = $("#unit_tel_guo").val();//公司电话国
		...
		var unit_fax_qu = $("#unit_fax_qu").val()//公司传真区
		var tel_qu = $("#tel_qu").val()//手机区
		baseList.user_name = $("#user_name").val();
		baseList.email = $("#email").val();
		baseList.credit_status = $("#credit_status").val();
		baseList.credit_time = $("#credit_time").val();
		baseList.gender = $("#gender").val();
		baseList.txtinput = $("#txtinput").val();
		baseList.department_id = $("#deparitment").val();
		baseList.credit_type = $("#credit_type").val();
		baseList.credit_title = $("#credit_title").val();
		baseList.basic_level = $("#basic_level").val();
		baseList.graduation_time = $("#graduation_time").val();
		baseList.education_id = $("#education_id").val();
		baseList.degree_id = $("#degree_id").val();
		baseList.province_id = $("#province_id").val();
		baseList.city_id = $("#city_id").val();
		baseList.org_id = $("#org_id").val();
		baseList.job_id = $("#job_id").val();
		baseList.unit_address = $("#unit_address").val();
		baseList.card = $("#card").val();
		baseList.unit_tel = unit_tel_guo+'-'+unit_tel_qu+'-'+$("#unit_tel").val();//单位电话
		baseList.unit_fax = unit_fax_guo+'-'+unit_fax_qu+'-'+$("#unit_fax").val();//单位传真
		baseList.tel = tel_qu+'-'+$("#tel").val();//手机号
		baseList.ticket_id ="";
		baseList.rmbs ="";
		baseList.start_time="";
		baseList.end_time="";
		baseList.invoice_status=""
		baseList.pay_type = 5;
		baseList.diy_org = $("#diy_org").val();
		storage.setItem('baseArr', JSON.stringify(baseList));
		
		window.location.href="/ip_annual_meeting02"
	})
